...
\begin{enumerate}
\item The one-step transition matrix \textbf{P} that defines the state transition probabilities.
\item The reward vector \textbf{R}  that describes the reward received in each state. This value depends on customer purchases and marketing expenditures.
\item The value vector \textbf{V} that describes  the expected present value of a customer in a specific sate. If the value is negative then that customer is effectively a loss for the company.
\end{enumerate}
